...
//            $insert = $db->delete('table.circle_follow')->rows(array('uid' => $uid, 'mid' => $mid)); // delete all
    ...
    {
        $db = Typecho_Db::get();
        return $db->fetchObject($db->select('mid', 'name', 'slug', 'description')->from('table.metas')->where('mid = ?', $mid));
    ...
    public static function getHotCircles()
    {
        ...
        foreach ($hots as $val) {
            ...
            array_push($data, [
                'mid' => $obj->mid,
                'topic_name' => $obj->name,
                'cover_image' => self::parseDesc2Img($obj->description),
            ]);
        }
        return $data;
    }
    ...
    public static function getHotPosts($page, $pageSize=10)
    {
//        Typecho_Widget::widget('Widget_Contents_Modify');
    }
}